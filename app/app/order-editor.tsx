...
function ItemsEditor({ items, setItems, services, setServices, presets, returnPresetId, setReturnPresetId }: {
  ...
  return <>
    <div className="order-items">{items.map((item, index) => { const service = services.find((entry) => entry.id === item.serviceId); const options = service?.options ?? []; const option = options.find((entry) => entry.id === item.optionId); const perSquareMeter = option?.pricing_mode === "per_m2"; const priceMissing = String(item.unitPrice).trim() === ""; return <div className="order-item-editor" key={item.key}>
      ...
      <div className="form-grid two"><label>Serviço <b className="required-mark">obrigatório</b><select value={item.serviceId} onChange={(event) => chooseService(item.key, event.target.value)}><option value="">Selecione</option>{services.filter((entry) => entry.active).map((entry) => <option key={entry.id} value={entry.id}>{entry.name}</option>)}</select><button type="button" className="text-action" onClick={() => setQuick({ itemKey: item.key, kind: "service", name: "", price: "", mode: "fixed", duration: 60 })}><Plus /> Serviço não cadastrado? Criar agora</button></label>
      <label>Modelo / categoria <span>opcional</span><select value={item.optionId} onChange={(event) => chooseOption(item.key, event.target.value)}><option value="">Sem modelo</option>{options.filter((entry) => entry.active).map((entry) => <option key={entry.id} value={entry.id}>{entry.name}{entry.sale_price === null ? " · preço na hora" : ` · ${money(entry.sale_price)}`}</option>)}</select>{item.serviceId && <button type="button" className="text-action" onClick={() => setQuick({ itemKey: `${item.key}::${item.serviceId}`, kind: "option", name: "", price: item.unitPrice, mode: "fixed", duration: item.duration })}><Plus /> Criar modelo deste serviço</button>}</label></div>
      ...
      {perSquareMeter ? <div className="dimension-box"><div><strong>Medidas</strong><small>Informe largura e comprimento para calcular o m².</small></div><div className="form-grid dimensions"><label>Largura (m)<input inputMode="decimal" type="number" min="0.01" step="0.01" value={item.width} onChange={(event) => changeDimension(item.key, "width", event.target.value)} /></label><span>×</span><label>Comprimento (m)<input inputMode="decimal" type="number" min="0.01" step="0.01" value={item.length} onChange={(event) => changeDimension(item.key, "length", event.target.value)} /></label><div className="sqm-result"><small>Total</small><strong>{item.quantity || 0} m²</strong></div></div></div> : <label>Quantidade<input type="number" min="0.01" step="0.01" value={item.quantity} onChange={(event) => changeItem(item.key, { quantity: Number(event.target.value) })} /></label>}
      <div className="form-grid item-values"><label className={priceMissing ? "order-price-field needs-price" : "order-price-field"}><span>Preço {perSquareMeter ? "por m²" : "desta ordem"} {priceMissing && <b>Defina agora</b>}</span><div className="money-field"><i>R$</i><input type="number" min="0" step="0.01" value={item.unitPrice} onChange={(event) => changeItem(item.key, { unitPrice: event.target.value })} placeholder="0,00" /></div><small className="field-help">Este preço pode ser diferente do preço padrão do modelo.</small></label><label><span>Duração (min)</span><input type="number" min="5" value={item.duration} onChange={(event) => changeItem(item.key, { duration: Number(event.target.value) })} /></label><label className="order-discount-field"><span>Desconto do item</span><div className="compound-input"><select value={item.discountType} onChange={(event) => changeItem(item.key, { discountType: event.target.value as "fixed" | "percent" })}><option value="fixed">R$</option><option value="percent">%</option></select><input type="number" min="0" step="0.01" value={item.discountValue} onChange={(event) => changeItem(item.key, { discountValue: Number(event.target.value) })} /></div></label></div>
    </div>; })}</div>
...
export function OrderDialog({ clients, services, presets, defaultClientId, onClose, onSaved }: { clients: Client[]; services: Service[]; presets: ReturnPreset[]; defaultClientId?: string; onClose: () => void; onSaved: () => void | Promise<void> }) {
  ...
  const initialClient = localClients.find((client) => client.id === defaultClientId);
  ...
  const [inlineClient, setInlineClient] = useState(false); const [newClientName, setNewClientName] = useState(""); const [newClientPhone, setNewClientPhone] = useState(""); const [duplicate, setDuplicate] = useState<{ id: string; name: string; whatsapp: string } | null>(null);
  const [editingClient, setEditingClient] = useState(false); const [items, setItems] = useState<ItemDraft[]>([newItem()]);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const { data: conflicts, error: conflictError } = await supabase.rpc("yan_check_conflicts", { p_start: startsAt.toISOString(), p_end: endsAt.toISOString(), p_exclude_order_id: null }); if (conflictError) throw conflictError; if (Array.isArray(conflicts) && conflicts.length && !window.confirm(`Existe conflito com ${conflicts.length} atendimento(s). Agendar mesmo assim?`)) { setBusy(false); return; }
...
export function EditOrderDialog({ order, clients, services, presets, onClose, onSaved }: { order: Order; clients: Client[]; services: Service[]; presets: ReturnPreset[]; onClose: () => void; onSaved: () => void | Promise<void> }) {
  const { localClients, setLocalClients, localServices, setLocalServices } = useLocalData(clients, services);
  const [step, setStep] = useState(1); const [clientId, setClientId] = useState(order.client_id); const [editingClient, setEditingClient] = useState(false);
  const [items, setItems] = useState<ItemDraft[]>((order.items ?? []).map((item) => ({ key: item.id || draftKey(), serviceId: item.service_id, optionId: item.option_id ?? "", quantity: Number(item.quantity), unitPrice: String(item.unit_price), unitCost: Number(item.unit_cost), discountType: item.discount_type, discountValue: Number(item.discount_value), duration: Number(item.duration_minutes), width: item.width_m === null ? "" : String(item.width_m), length: item.length_m === null ? "" : String(item.length_m) })) || [newItem()]);
  const initialAddress = addressFromOrder(order); const [wantsAddress, setWantsAddress] = useState(hasAddress(initialAddress)); const [address, setAddress] = useState<AddressDraft>(initialAddress);
  const [scheduledDate, setScheduledDate] = useState(datePart(order.scheduled_start)); const [scheduledTime, setScheduledTime] = useState(timePart(order.scheduled_start));
  const matchingPreset = presets.find((preset) => preset.value === order.return_value && preset.unit === order.return_unit); const [returnPresetId, setReturnPresetId] = useState(matchingPreset?.id ?? defaultPresetId(presets));
  const [notes, setNotes] = useState(order.notes ?? ""); const [discountType, setDiscountType] = useState<"fixed" | "percent">(order.discount_type); const [discountValue, setDiscountValue] = useState(Number(order.discount_value));
  const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const selectedClient = localClients.find((client) => client.id === clientId); const selectedPreset = presets.find((preset) => preset.id === returnPresetId); const validItems = items.filter((item) => item.serviceId && item.quantity > 0); const totals = itemTotals(items, discountType, discountValue);
  ...
  async function save(event: FormEvent) {
    ...
    try {
      const startsAt = new Date(`${scheduledDate}T${scheduledTime}:00`); const endsAt = new Date(startsAt.getTime() + totals.duration * 60_000);
      const { data: conflicts, error: conflictError } = await supabase.rpc("yan_check_conflicts", { p_start: startsAt.toISOString(), p_end: endsAt.toISOString(), p_exclude_order_id: order.id }); if (conflictError) throw conflictError; if (Array.isArray(conflicts) && conflicts.length && !window.confirm(`Existe conflito com ${conflicts.length} atendimento(s). Salvar mesmo assim?`)) { setBusy(false); return; }
  ...
  return <Modal title={`Editar ordem #${order.order_number}`} subtitle="Altere cliente, serviços, modelos, preços, endereço, data, hora e observações." onClose={onClose} wide panelClassName="modal-order"><form className="modal-form order-wizard" onSubmit={save}><WizardProgress step={step} labels={["Cliente", "Serviços", "Local e agenda"]} />
