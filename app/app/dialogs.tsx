...
export function CompleteDialog({ order, settings, onClose, onSaved }: { order: Order; settings: YanSettings | null; onClose: () => void; onSaved: () => void | Promise<void> }) {
  ...
  return <Modal title={`Concluir ordem #${order.order_number}`} subtitle={`${order.client?.name ?? "Cliente"} · ${money(order.total)}`} onClose={onClose} panelClassName="modal-complete"><div className="modal-form complete-wizard"><WizardProgress step={step} labels={["Fotos", "Pagamento", "Finalizar"]} />
    ...
    {step === 3 && <section className="wizard-step review-step"><div className="wizard-heading"><span><Check /></span><div><strong>Confirmar conclusão</strong><p>O horário exato de conclusão será destacado no card.</p></div></div><div className="finish-summary"><article><small>Cliente</small><strong>{order.client?.name}</strong></article><article><small>Valor</small><strong>{money(order.total)}</strong></article><article><small>Retorno</small><strong>{order.return_label ?? "6 meses"}</strong></article><article><small>Garantia no PDF</small><strong>{settings?.warranty_value ?? 6} {settings?.warranty_unit === "days" ? "dias" : "meses"}</strong></article><article><small>Fotos depois</small><strong>{files.length}</strong></article><article><small>Pagamento</small><strong>{mode === "paid" ? "Recebido" : mode === "due" ? "A receber" : `${installments} parcelas`}</strong></article></div><div className="pdf-preview-note"><FileDown /><span><strong>PDF bonito e pronto para o cliente</strong><small>Após concluir, você poderá baixar ou compartilhar diretamente.</small></span></div></section>}
    {error && <div className="form-alert error"><AlertTriangle />{error}</div>}<div className="modal-actions wizard-actions"><button type="button" className="button-secondary" onClick={() => step === 1 ? onClose() : setStep(step - 1)}>{step === 1 ? "Cancelar" : <><ArrowLeft /> Voltar</>}</button>{step < 3 ? <button type="button" className="button-admin-primary" onClick={() => setStep(step + 1)}>Continuar <ArrowRight /></button> : <button className="button-admin-primary" disabled={busy} onClick={() => void finish()}>{busy ? <Loader2 className="spin" /> : <Check />}{busy ? "Concluindo..." : "Concluir e gerar PDF"}</button>}</div>
  </div></Modal>;
}
